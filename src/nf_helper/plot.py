...
class PlotKmer:
    ...
    def make_plot_kmer(self):
        ...
        plot_df = kmer_df.melt(id_vars=["length"], value_vars=k_cols, var_name="k", value_name="kmer_count")


        kmer_plot_df = plot_df.groupby(by='k').sum().drop(columns=['length']).reset_index()
        kmer_plot_df['k'] = kmer_plot_df['k'].apply(lambda x:x.split("_")[-1]).astype(int)
        kmer_plot_df = kmer_plot_df.sort_values(by='k')

        # Numerical derivative dkmer_count/dk at each k
        kmer_plot_df["dkmer_dk"] = np.gradient(
            kmer_plot_df["kmer_count"].to_numpy(),
            kmer_plot_df["k"].to_numpy()
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument('-i','--input_file', dest='input_file', help='Input file with kmer distribution')
