...
class Kmer:
    ...
    def read_file(self):
        df = pd.read_csv(
            self.in_file, sep="\t", header=None, names=["read_id", "length"]
        )
        length_counts = df["length"].value_counts().sort_index()
        len_df = pd.DataFrame(
            {"length": length_counts.index, "count": length_counts.values}
        )

        return len_df

    def kmer_dist(
        self, len_df: pd.DataFrame, min_kmer: int = 20, step_size: int = 1
    ) -> pd.DataFrame:
        kmer_df = len_df.copy()
        ...
        kmer_cols = {}
        for k in kvals:
            kmer_cols[f"k_{k}"] = (len_df["length"] - k + 1).clip(lower=0) * len_df[
                "count"
            ]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Calculate k-mer distribution from read lengths"
    )
    parser.add_argument(
        "-i",
        "--input_file",
        dest="input_file",
        help="Input file with read_id and length",
    )
    parser.add_argument(
        "-m",
        "--min-kmer",
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Step size for k-mer calculation",
    )
    parser.add_argument(
