...
def build_kraken2(build_name: str, genome_names: list, tagged_dir: str, db_dir: str, taxonomy_dir: str, cfg: dict, log_dir: str, logger):
    ...
    for genome_name in genome_names:
        ...
        run(["kraken2-build", "--add-to-library", tagged_fna, "--db", db_dir],
    ...
    if sentinel(done_flag):
        ...
        run(["kraken2-build", "--build",
             ...
             "--kmer-len",        str(cfg["kmer_len"]),
             "--minimizer-len",   str(cfg["minimizer_len"]),
             "--minimizer-spaces",str(cfg["minimizer_spaces"])],
...
def main():
    ...
    parser.add_argument("--builds",  nargs="*", help="Which db_builds to run (default: all)")
    ...
    for genome_name in needed_genomes:
        ...
        dl_dir = download_genome(genome_name, genome,
                                 downloads_dir, log_dir)
        if dl_dir is None:
            logger.warning(f"Skipping {genome_name} — manual download needed.")
            continue
        tag_fasta(genome_name, genome["taxid"],
                  dl_dir, tagged_dir, log_dir)
    ...
    for build_name in builds_to_run:
        ...
        build_kraken2(build_name, genome_names,
                      tagged_dir, db_dir, taxonomy_dir,
                      cfg, log_dir, logger)
