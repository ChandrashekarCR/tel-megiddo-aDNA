...
def tag_fasta(name:str, taxid: int, download_dir: str, tagged_dir: str, log_dir:str) -> str:
    """
    ...
    if not fastas:
        raise FileNotFoundError(
            f"No FASTA files found for {name} in {download_dir}"
        )

    logger.info(f"Tagging {name} (taxid={taxid}) "
                f"from {len(fastas)} file(s) -> {out_fna}")
